...
| Stage 8 | 意识体系统 — Context 分层管理、自我感知/执行意识、记忆系统、虚拟工具、子意识派生 | ✅ 完成 |
...
- `packages/shared/src/types/consciousness.ts` — 意识体类型（ConsciousnessNode、MemoryEntry、ContextSnapshot 等）
- `packages/shared/src/logger.ts` — 全局调试 Logger
- `packages/agent/src/consciousness-manager.ts` — 意识体树管理器（创建、状态更新、压缩、记忆检索）
- `packages/agent/src/consciousness-prompts.ts` — 意识体系统提示词模板
- `packages/agent/src/memory-store.ts` — 记忆存储与检索（.tramber/memory/）
- `packages/agent/src/context-storage.ts` — 意识体 Context 文件夹存储（.tramber/contexts/）
- `packages/agent/src/virtual-tools/` — 虚拟工具（spawn_sub_task、recall_memory 等）
